...
def run_evaluation(signs_data, test_mode_fields, report_file):
    ...
    Uses a strict checking method based on the LLM's formatted output.
    """
    report_header = f"STARTING TEST: Using {len(test_mode_fields)} fields: {', '.join(test_mode_fields)}"
    print("\n" + "="*50)
    print(report_header)
    print("="*50)
    ...
    for i, (word, details) in enumerate(signs_data.items()):
        ...
        print(f"Assistant's Response: {llm_answer}")
        
        cleaned_llm_word = re.sub(r'[^A-Z0-9]', '', llm_answer.upper())
        cleaned_expected_word = re.sub(r'[^A-Z0-9]', '', word.upper())

        if cleaned_llm_word == cleaned_expected_word:
    ...
    # --- Generate Report ---
    summary_lines = [
        f"TEST SUMMARY: {len(test_mode_fields)} fields mode\n",
...
if __name__ == "__main__":
    ...
    report_filename = f"test_report_{timestamp}_all.txt"

    with open(report_filename, 'w', encoding='utf-8') as report_file:
        print(f"Starting evaluation. Results will be saved to '{report_filename}'")
        report_file.write(f"ASL Sign Recognizer Evaluation Report - {timestamp}\n\n")
